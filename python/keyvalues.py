...
COMMENT_LOCATION_ABOVE=0
COMMENT_LOCATION_SAMELINE=1
COMMENT_LOCATION_DEFAULT=0

spam_comment_info = False

class Comment:
	content=''
	location=COMMENT_LOCATION_DEFAULT
	
	def __init__(self,content,location=COMMENT_LOCATION_DEFAULT):
...
class KeyValues(collections.MutableMapping):
	...
	def set_comment_list(self, key, index, value, location=COMMENT_LOCATION_DEFAULT):
		if key not in self.comments or type(self.comments[key]) is not list:
			self.comments[key]=[]
		if len(self.comments[key]) < len(self._children[key]):
			while len(self.comments[key]) < len(self._children[key]):
				self.comments[key].append('')
			#print("self.comments['{0}'] = len {1} (needs to be len {2})".format(key,len(self.comments[key]),len(self._children[key])))
		#print("self.comments['{0}'][{1}] = Comment('{2}',{3})".format(key,index,value,location))
		self.comments[key][index]=Comment(value, location)
		
	def get_comment_list(self, key, index):
		if key in self.comments:
			if index < len(self.comments.key):
				return self.comments[key][index]
		return None
	...
	def stringify(self, indentation=True, inline=False, space="\t"):
		...
		if not indentation:
			space = ""
		result = ''
		if spam_comment_info:
			result += '// ' + str(type(self.comments)) + ' = ' + repr(self.comments) + line_break
	...
	def _stringify_recursive(self, indentation, line_break, space, indentation_level):
		...
		for key in self._children:
			...
			if spam_comment_info:
				result += prefix_in + '// ' + str(type(comment)) + ' = ' + repr(comment) + line_break
			result += prefix_in + self._escape(str(key)) + str(comment)
			if type(value) is list:
				for i in range(len(value)):
					l_comment=Comment('',-1)
					if i < len(comment_list):
						if comment_list[i]!='':
							l_comment=comment_list[i]
					if i > 0:
						if spam_comment_info:
							result += line_break + prefix_in + '// ' + str(type(l_comment)) + ' = ' + repr(l_comment)
						if l_comment.location==COMMENT_LOCATION_ABOVE:
							result += line_break + prefix_in + str(l_comment)
						result += line_break
						result += prefix_in + self._escape(str(key))
						if l_comment.location==COMMENT_LOCATION_SAMELINE:
							result += ' '+str(l_comment)
					if type(value[i]) is KeyValues:
						result += line_break
						if spam_comment_info:
							result += '// [internal] ' + str(type(value[i].comments)) + ' = ' + repr(value[i].comments) + line_break
				...
				result += line_break
				if spam_comment_info:
					result += '// [internal] ' + str(type(value.comments)) + ' = ' + repr(value.comments) + line_break
